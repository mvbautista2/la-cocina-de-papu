...
export default function Product(props) {
  ...
  const deleteProductCart = async () => {
    const response = await deleteProductCartApi(product.slug);
    if (response) setReloadCart(true);
  };

  const increaseProductCart = async () => {
    const response = await increaseProductCartApi(product.slug);
    if (response) setReloadCart(true);
  };

  const decreaseProductCart = async () => {
    const response = await decreaseProductCartApi(product.slug);
    if (response) setReloadCart(true);
  };
  return (
